...
[6. 우주방어 구성요소](#6-우주방어-구성요소)  
[7. 게임 시스템 디자인](#7-게임-시스템-디자인)  
[8. 개발 요구사항 & 흐름도](#8-개발-요구사항--흐름도)  
[9. 개발작업 일정(6주)](#9-개발작업-일정-6주)  
...
(9)게임 종료 후 화면에서 우측 상단에 메인 메뉴로 돌아가기 버튼을 눌러 돌아가거나 사용자 조작이 없을 경우 15초 후에 자동으로 메뉴로 보낸다.~~  
(10)단계가 증가 할 수록 유닛 수가 점점 증가하고 단계 수는 15단계이다.  
(11)단계가 증가할 때마다 알람 소리를 넣는다.  
(12)게임 종료시 막아낸 단계 수(점수)가 화면 중앙에 표시된다.  
(13)게임 종료시 성공과 실패를 구분하기 위해 "성공"과 "실패" 문구를 상황에 맞게 띄운다.
(14)유닛 전투시 타격 효과음과 상황에 맞는 시각적 효과를 넣는다.  
(15)유닛들의 부위 별로 행동과 상태에 따라 적절한 애니메이션을 부여한다.  
(16)생산한 유닛이나 사용한 자원 양이 1.5초 동안 표시 되도록 나타내는 효과를 부여한다.  
(17)생성된 적 유닛이 길을 찾아 장애물을 피해 사령부로 도달하도록 한다.(네비게이션)  
...
요구사항  
(1) 시작화면, 점수화면, 게임화면 총 3개의 화면이 있다.  (100%)  
(2) 게임 시작 버튼을 누르면 게임을 바로 시작한다. (100%)  

### 2주차  

요구사항  
(3)게임화면에서는 좌측 상단에 단계(Wave)가 표시되고 우측 상단에 점수(Score)가 표시 된다. 하단에 생성 할 수 있는 건물 및 유닛 패널이 있다. (100%)  
(4)유닛 생성은 패널에서 생성 위치로 드래그 앤 드롭을 이용한다. (75%)  

현재 드래그 앤 드롭이 아닌 버튼 클릭으로 구현해놨음  

### 3주차  

요구사항  
(5)유닛과 건물 선택은 마우스 좌클릭, 이동과 공격은 마우스 우클릭을 이용한다.  (25%)  
(6)유닛들은 사정거리 안에 적을 감지하여 공격한다.  (0%)  

유닛 건물 선택과 카메라 이동쪽에 수많은 인스턴스 오류가 생김. 유닛 선택 해당 코드를 삭제함.  

### 4주차  

요구사항  
(5)유닛과 건물 선택은 마우스 좌클릭, 이동과 공격은 마우스 우클릭을 이용한다. (75%)  
(6)유닛들은 사정거리 안에 적을 감지하여 공격한다. (100%)     

### 5주차  

해당 요구사항 :  

(7) 단계가 증가 할 수록 유닛 수가 점점 증가하고 3단계까지 프로토타입으로 만들어 새로운 유닛과 추가 유닛이 생성 되도록 한다. (75%)  
(8) 게임 종료 조건은 사령부가 깨지면 패배하거나 모든 단계를 막았을 때 승리로 끝난다. (100%)  

설명 : 
  
적 유닛이 사령부에 충돌함에 따라 목숨(Life) 차감, 0이 되면 게임 오버  
유닛 생성 포인트를 여러개를 만들고 유닛이 다양한 곳에서 공격해 오게 만들 것  
아군 유닛의 이동과 공격 제어를 구현  

### 6주차  
    
해당 요구사항 :  
  
(9) 게임 종료 후 화면에서 우측 상단에 메인 메뉴로 돌아가기 버튼을 눌러 돌아가거나 사용자 조작이 없을 경우 15초 후에 자동으로 메뉴로 보낸다. (100%)  
(7) 새로운 유닛과 추가 유닛이 생성 (100%)  
  
설명 :   
  
총알이 발사 된 시점의 적 포지션으로 날아가게 수정한다.  
5주차 기반으로 만들어진 스테이지에서 새로운 유닛을 추가하여 생성되게 한다.  
Score화면에서 버튼을 눌렀을때와 시간이 지났음에 따라 Menu화면으로 돌아가게 한다.  
